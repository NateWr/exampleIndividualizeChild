...
Custom full-text extractors are named `<Journal>FullText.inc.php`. Each extractor is written to extract the article's full text from the corresponding sample HTML file.
...
3. Separate the main article from the references and handle endnotes or reference sections.
...
5. Make sure all headings have appropriate `id` attributes, so that the table of contents can be constructed.
6. Replace dependent file URLs (use the helpful methods `getDependentFiles()` and `replaceDependentFileUrls()`).
